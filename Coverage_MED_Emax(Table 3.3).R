...
seed=readRDS("C:/Users/saha/Desktop/Weighted Regression MED/seed.RDS")
Delta=0.4
...
registerDoSNOW(cl)
#Segment 1: MED Inference

nsim=2000
SSIZE=c(25,50,75,100)
level=0.95
MED<- f_inverse(Delta+0.32,c( e0=0.32,eMax=0.74,ed50=0.14),model=model)
#seed=sample(1:5000000,nsim)
system.time(CfitAll<-foreach(s=1:4) %:%
              foreach(i=1:nsim)%dopar%
              { 
                set.seed(seed[i])
                data=MCPMod::genDFdata(model, c( e0=0.32,eMax=0.74,ed50=0.14), c(0,0.05,0.2,0.6,1), SSIZE[s],0.65)
                fit <- fitMod(dose, resp,data,model = model, bnds = defBnds(1)[[model]])
                wfit<-Weighted_MED(data,model=model,Delta=Delta,weights="weights6")
                ...
                wfit2<-Weighted_fitMod(data,model=model,wts="weights6",maxit = 100,tol = 0.001,off=0.2,convergence_criteria="MED",Delta=Delta)
                ...
                CfitwIRNLS<-confintwIRLS_MED(level,wfit2$m$coefs,model,dose,data,weights="weights6",S=SI,Delta,MED_est,ssize=rep(SSIZE[s],length(dose)))
                ...
                CfitwRR<-confintwRR_MED(level=level,wfit$coefs,model=model,dose,data,weights="weights6",S=SI,Delta=Delta,wfit$MED,ssize=rep(SSIZE[s],length(dose)))
                # emxBobj <- bFitMod(dose, drFit, S=S,model=model, type = "bootstrap",nSim=1000, bnds = c(0.01, 1))
...
for(k in 1:length(SSIZE))
{
  ...
  Cov[k,]=sapply(1:3,function(j)mean(dat[j,], na.rm=TRUE))
}
...
cov_prob=sapply(1:4,function(i)sum(apply(Cfit[[i]],2,IntervalCont,x=MED))/nsim)

#PL Approach
source("C:/Users/saha/Desktop/Jyoti_Run/Prof_Lik_Bayn.R") 
system.time(CfitPL<-foreach(s=1:4)%:%
              foreach(i=1:nsim, .combine='cbind') %dopar%
              {set.seed(seed[i])
                data=MCPMod::genDFdata(model, c( e0=0.32,eMax=0.74,ed50=0.14), c(0,0.05,0.2,0.6,1), SSIZE[s],0.65)
...
cov_prob=sapply(1:4,function(i)sum(apply(CfitPL[[i]],2,IntervalCont,x=MED))/nsim)
